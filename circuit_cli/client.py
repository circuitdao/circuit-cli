...
class CircuitRPCClient:
    ...
    async def _make_api_request(
        ...
        """
        try:
            json_data_excl_spks = deepcopy(json_data)
            json_data_excl_spks.pop("synthetic_pks", None)
            log.info(f"Making request to {method} {endpoint} with params {params} and json_data (excl spks) {json_data_excl_spks}")
